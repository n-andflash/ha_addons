...
* 집에 어떤 장치가 있는지는(어떤 장치를 스캔해야 하는지는) 관리자 페이지에 따로 설정되어 있습니다.
  ...
  * 네트워크 관련한 설정을 수정하는 경우에는 A/S 접수가 필요할 수 있으니 주의 바랍니다.
* 구형 월패드(EZON) 중에는 장치 스캔 없이 항상 Keepalive 패킷을 시도하는 경우도 있습니다.
...
| AC 79 `GR` 00 `PT` | B0 79 `GR` `TG` `PT` | - | - | 조명 상태 확인 (구형 월패드) |
| AC 79 00 `GR` `PT` | B0 1C 00 00 2C | AC 1C 01 26 `U1 U1` `PT` | B0 79 00 00 49 | ? |
...
* `ZZ`: 현재 소모량 (10진수 ZZZZZZ)
    * 단위는 전기는 W, 가스/수도는 /100 m³/h 입니다. (집에 따라 다른 경우 있습니다. 전기가 /100 W인 경우 등)
